...
def calc(n,m,ttuser,ttitem,pre,ttrating,atk=5):
    ...
    # print(ans)
    return [ans[0],ans[1], ans[3], ans[4]]
...
def auc(vector_predict, vector_true, device = torch.device("cuda" if torch.cuda.is_available() else "cpu")):
...
def evaluate(vector_Predict, vector_Test, metric_names, users = None, items = None):
    ...
    if 'Recall_Precision_NDCG@' in metric_names: 
        ...
        results['MRR'] =  Recall_Precision_NDCG[2]
        results['NDCG'] =  Recall_Precision_NDCG[3]
    #print(results)
