...
def load_dataset(data_name='yahooR3', type = 'explicit', unif_ratio = 0.05, seed=0, implicit=True, threshold=4, device = 'cuda'): 
    ...
    if type == 'list': 
        ...
        train_matrix = sp.csr_matrix((user_df['rating'], (user_df['uid'], user_df['iid'])), shape=(m, n), dtype='float32')
        train = sparse_mx_to_torch_sparse_tensor(train_matrix).to(device)
    ...
    item_pop = get_popularity('yahooR3')
    return train, unif_train, validation, test, item_pop

def get_popularity(data_name='ml-1m'):
    item_list = []
    path = os.path.dirname(os.path.dirname(__file__)) + '/datasets/' + data_name
    path1 = path + '/random.txt'
    path2 = path + '/user.txt'
    with open(path1, 'r') as f:
        ...
        for i, line in enumerate(lines):
            ...
            item_list.append(i)
    with open(path2, 'r') as fn:
        lines = fn.readlines()
        for i, line in enumerate(lines):
            u,i,r = line.strip().split(',')
            item_list.append(i)

    item_inter_num = Counter(item_list)
    #print(item_inter_num)
