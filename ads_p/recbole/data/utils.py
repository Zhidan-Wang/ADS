...
def create_dataset(config, gene_batch):
    ...
    if hasattr(dataset_module, config['model'] + 'Dataset'):
        ...
        if model_type == ModelType.SEQUENTIAL:
            if config['tongji'] == True:
                from .dataset import Sequential11Dataset
                return Sequential11Dataset(config, gene_batch)
            else:
                from .dataset import SequentialDataset
                return SequentialDataset(config, gene_batch)
            ...
            #print("Dataset@@@@@@@@@")
            return Dataset(config, gene_batch)
...
def data_preparation(config, dataset, save=False):
    ...
    """
    model_type = config['MODEL_TYPE'] #ModelType.GENERAL
    ...
    kwargs = {}
    if config['training_neg_sample_num']: # 1
        ...
        train_distribution = config['training_neg_sample_distribution'] or 'uniform' #uniform
...
def dataloader_construct(
    ...
    for key, value in kwargs.items(): #sampler, neg_sample_args
        key = [key] * len(dataset)
    ...
    dataloader = get_data_loader(name, config, eval_setting) #eval:GeneralFullDataLoader // train:GeneralNegSampleDataLoader
