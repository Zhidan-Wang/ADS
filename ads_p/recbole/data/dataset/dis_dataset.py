...
class DisDataset(Dataset):
    def __init__(self, config, data_file, inter_file, real_dataset, np_ratio, saved_dataset=None):
        super().__init__(config, gene_batch='', saved_dataset=saved_dataset)
        self.g_file = data_file
        self.g_inter_file = inter_file
        gene = self.get_gene_inter(self.g_file, self.g_inter_file, config['dataset'])

        self.gene_len = len(gene)
        self.real_len = self.gene_len / np_ratio


        self.inter_feat = real_dataset.inter_feat
        self.inter_feat = cat_interactions([self.inter_feat, gene])
        self.prepare_data_augmentation()

        print("total!!!!!!!!!!!!!!", len(self.uid_list))
        pos_tot = 0
